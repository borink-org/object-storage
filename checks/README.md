...
- `no-allocator/` builds the Rust API a Rust consumer calls directly, `no_std` and with no global allocator, so a path that allocated would not compile. It registers both of `borink-crypto`’s checksum providers and computes one, so the `md-5` adapter is covered as well as the encoder. The image above covers the same code through the C ABI; this covers it through the Rust one.
