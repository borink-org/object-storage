...
The header is committed. `generate-header.sh` rewrites it with the cbindgen that `flake.nix` pins, and CI fails on a diff. It includes only `<stdbool.h>`, `<stddef.h>` and `<stdint.h>`, and the archive calls no allocator; `checks/freestanding` links it into a Cortex-M7 image with no C library to check both, along with the C++ headers, which reach no allocator either.
