...
/// Writes an entity tag from a listing in the form that a condition takes.
///
/// Azure lists an entity tag without the quotes that the `ETag` header carries,
/// and a condition takes the quoted form. This copies `listed` into `into`,
/// adding the quotes unless it already carries them or is a weak tag, and
/// returns what it wrote. Returns [`None`] if `into` is too small, which takes
/// two bytes more than `listed`.
///
