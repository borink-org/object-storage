...
#[test]
fn native_blocks_preserve_selectors_and_list_kinds() {
    use borink_object_storage_proto::HeaderSpan;
    use borink_object_storage_proto::azure::{
        BlockListKind, BlockRef, BlockSource, CommitBlocks, ListBlocks,
    };
    ...
    let blocks = [
        BlockSource::Committed,
        BlockSource::Uncommitted,
        BlockSource::Latest,
    ]
    .map(|source| BlockRef { id: "YQ==", source });
    let request = blobs
        .encode_commit_blocks(
            &mut buf,
            &mut headers,
            &CommitBlocks::new("object"),
            &blocks,
            &now(),
        )
        .unwrap();
    assert!(
        std::str::from_utf8(request.payload().bytes().unwrap())
            .unwrap()
            .contains(
                "<Committed>YQ==</Committed><Uncommitted>YQ==</Uncommitted><Latest>YQ==</Latest>",
            )
    );
    let plan = ListBlocks {
        key: "object",
        kind: BlockListKind::All,
        snapshot: None,
        version: None,
    };
    let request = blobs
        .encode_list_blocks(&mut buf, &mut headers, &plan, core::iter::empty(), &now())
        .unwrap();
    assert!(request.url().contains("blocklisttype=all"));
}
