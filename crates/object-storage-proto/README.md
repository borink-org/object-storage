...
For C++, [`crates/object-storage-cxx`](../object-storage-cxx) is a binding built on this crate, and [`hosts/cxx-curl`](../../hosts/cxx-curl) is a libcurl host that uses it. The binding allocates once per client and never per request, and returns no `Result`, so an application built without exceptions can use it. Your application keeps its HTTP client, its buffers and its memory budget.
